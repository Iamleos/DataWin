<?php
    ...
    foreach ($yiren as $key => $value) {
        $yirenname = $value[0];
        $url_name = urlencode($yirenname);

        //指数概况
        $url = "http://index.haosou.com/index/overviewJson?area=%E5%85%A8%E5%9B%BD&q=".$url_name;
        $ch = curl_init($url);
        ...
        $indexdata = json_decode($result)->data[0];
        //如果没有数据，跳过该关键词
        if($indexdata == NULL){
            continue;
        }
        ...
        //入库
        mysqli_query($con, "insert into 360_yiren_qushi values('{$yirenname}', '{$monthindex}','{$week_index}','{$month_year_ratio}','{$month_chain_ratio}','{$search_lastday}',
                                                                '{$media_lastday}','{$data_date}','{$date}');");
        var_dump($yirenname);
        sleep(1.5);
