<?php
    ...
    foreach ($zongyi as $key => $value) {
        ...
        do {
            $json = NULL;
            $data = NULL;
            exec("python /var/www/html/zhishu/360_zhishu/python/getData_qushi.py '{$zongyiname}'",$json);
        ...
        */
        //入库
        mysqli_query($con, "insert into 360_zongyi_qushi values('{$zongyiname}', '{$monthindex}','{$week_index}','{$month_year_ratio}','{$month_chain_ratio}','NULL',
                                                                'NULL','{$data_date}','{$date}');");
        var_dump($zongyiname);
        sleep(1);
    }
