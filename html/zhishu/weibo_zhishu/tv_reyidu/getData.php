<?php
    ...
    $tv = mysqli_query($con, "select name from search_list;");
    ...
    foreach ($tv as $key => $value) {
        ...
        $tvname = $value[0];
        exec("python /var/www/html/zhishu/weibo_zhishu/tv_reyidu/getId.py $tvname", $id);
        sleep(1);
        if ($id[0] == 'error') {
            $id = NULL;
            continue;
        }
        exec("python /var/www/html/zhishu/weibo_zhishu/tv_reyidu/getData.py $id[0]", $json);
        sleep(1);
        ...
        //入库
        mysqli_query($con,"insert into wzs_tv_reyidu_region(name,ningxia,jiangxi,beijing,zhejiang,henan,guangdong,shanghai,jiangsu,sichuan,shandong,hubei,fujian,hunan,hebei,liaoning,chongqing,anhui,shanxi,
        hongkong,guangxi,yunnan,heilongjiang,shaanxi,taiwan,jilin,hainan,guizhou,tianjin,gansu,neimongol,xinjiang,qinghai,macau,xizang,time) values('{$tvname}','{$region_reyidu["ningxia"]["ct"]}','{$region_reyidu["jiangxi"]["ct"]}','{$region_reyidu["beijing"]["ct"]}',
        ...
        mysqli_query($con,"insert into wzs_tv_reyidu_user(name,ningxia,jiangxi,beijing,zhejiang,henan,guangdong,shanghai,jiangsu,sichuan,shandong,hubei,fujian,hunan,hebei,liaoning,chongqing,anhui,shanxi,
        hongkong,guangxi,yunnan,heilongjiang,shaanxi,taiwan,jilin,hainan,guizhou,tianjin,gansu,neimongol,xinjiang,qinghai,macau,xizang,time) values('{$tvname}','{$user_reyidu["ningxia"]["num"]}','{$user_reyidu["jiangxi"]["num"]}','{$user_reyidu["beijing"]["num"]}',
