    ...
    foreach ($tv as $key => $value) {
        ...
        $tvname = $value[0];
        exec("python /var/www/html/zhishu/weibo_zhishu/tv_shuxin/getId.py $tvname", $id);
        sleep(1);
        if ($id[0] == 'error') {
            $id = NULL;
            continue;
        }
        exec("python /var/www/html/zhishu/weibo_zhishu/tv_shuxin/getData.py $id[0] $id[1]", $json);
        sleep(1);
        ...
        //插入库
        mysqli_query($con, "insert into wzs_tv_shuxin values('{$tvname}','{$sex["man"]}','{$sex["woman"]}','{$age["0-12"]}','{$age["12-18"]}','{$age["19-24"]}'
