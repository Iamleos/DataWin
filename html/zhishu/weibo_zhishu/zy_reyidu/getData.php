<?php
    ...
    $zy = mysqli_query($con, "select name from linshi_word");
    $zy = mysqli_fetch_all($zy);
    mysqli_close($con);
    //入库
    $host="56a3768226622.sh.cdb.myqcloud.com";
    $name="root";
    ...
    foreach ($zy as $key => $value) {
        ...
        $zyname = $value[0];
        exec("python /var/www/html/zhishu/weibo_zhishu/zy_reyidu/getId.py $zyname", $id);
        sleep(1);
        if ($id[0] == 'error') {
            $id = NULL;
            continue;
        }
        exec("python /var/www/html/zhishu/weibo_zhishu/zy_reyidu/getData.py $id[0]", $json);
        sleep(1);
        ...
        //入库
        mysqli_query($con,"insert into wzs_zy_reyidu_region(name,ningxia,jiangxi,beijing,zhejiang,henan,guangdong,shanghai,jiangsu,sichuan,shandong,hubei,fujian,hunan,hebei,liaoning,chongqing,anhui,shanxi,
        hongkong,guangxi,yunnan,heilongjiang,shaanxi,taiwan,jilin,hainan,guizhou,tianjin,gansu,neimongol,xinjiang,qinghai,macau,xizang,time) values('{$zyname}','{$region_reyidu["ningxia"]["ct"]}','{$region_reyidu["jiangxi"]["ct"]}','{$region_reyidu["beijing"]["ct"]}',
        ...
        mysqli_query($con,"insert into wzs_zy_reyidu_user(name,ningxia,jiangxi,beijing,zhejiang,henan,guangdong,shanghai,jiangsu,sichuan,shandong,hubei,fujian,hunan,hebei,liaoning,chongqing,anhui,shanxi,
        hongkong,guangxi,yunnan,heilongjiang,shaanxi,taiwan,jilin,hainan,guizhou,tianjin,gansu,neimongol,xinjiang,qinghai,macau,xizang,time) values('{$zyname}','{$user_reyidu["ningxia"]["num"]}','{$user_reyidu["jiangxi"]["num"]}','{$user_reyidu["beijing"]["num"]}',
