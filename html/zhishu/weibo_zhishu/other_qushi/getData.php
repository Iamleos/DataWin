...
//调用python爬取数据
    ...
    foreach ($other as $key => $value) {
        $othername = $value[0];
        exec("python /var/www/html/zhishu/weibo_zhishu/other_qushi/getId.py $othername", $id);
        sleep(1);
        if ($id[1] == 'error') {
            $id = NULL;
            continue;
        }
        exec("python /var/www/html/zhishu/weibo_zhishu/other_qushi/getData.py $id[1] $sdate $edate", $json);
        sleep(1);
        ...
        //入库
        mysqli_query($con, "insert into wzs_other_qushi values('{$othername}', '{$zt_last->value}', '{$yd_last->pc}', '{$yd_last->mobile}', '{$zt_last->day_key}', '{$date}');");
        var_dump($othername);
        $id = NULL;
        $json = NULL;
    }
