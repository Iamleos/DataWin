    ...
    foreach ($film as $key => $value) {
        ...
        $filmname = $value[0];
        exec("python /var/www/html/zhishu/weibo_zhishu/film_shuxin/getId.py $filmname", $id);
        sleep(1);
        if ($id[0] == 'error') {
            $id = NULL;
            continue;
        }
        exec("python /var/www/html/zhishu/weibo_zhishu/film_shuxin/getData.py $id[0] $id[1]", $json);
        sleep(1);
        ...
        //插入库
        mysqli_query($con, "insert into wzs_film_shuxin values('{$filmname}','{$sex["man"]}','{$sex["woman"]}','{$age["0-12"]}','{$age["12-18"]}','{$age["19-24"]}'
