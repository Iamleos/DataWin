<?php
    ...
    foreach ($film as $key => $value) {
        ...
        $filmname = $value[0];
        exec("python /var/www/html/zhishu/weibo_zhishu/film_reyidu/getId.py $filmname", $id);
        sleep(1);
        if ($id[0] == 'error') {
            $id = NULL;
            continue;
        }
        exec("python /var/www/html/zhishu/weibo_zhishu/film_reyidu/getData.py $id[0]", $json);
        sleep(1);
        ...
        //入库
        mysqli_query($con,"insert into wzs_film_reyidu_region(name,ningxia,jiangxi,beijing,zhejiang,henan,guangdong,shanghai,jiangsu,sichuan,shandong,hubei,fujian,hunan,hebei,liaoning,chongqing,anhui,shanxi,
        hongkong,guangxi,yunnan,heilongjiang,shaanxi,taiwan,jilin,hainan,guizhou,tianjin,gansu,neimongol,xinjiang,qinghai,macau,xizang,time) values('{$filmname}','{$region_reyidu["ningxia"]["ct"]}','{$region_reyidu["jiangxi"]["ct"]}','{$region_reyidu["beijing"]["ct"]}',
        ...
        mysqli_query($con,"insert into wzs_film_reyidu_user(name,ningxia,jiangxi,beijing,zhejiang,henan,guangdong,shanghai,jiangsu,sichuan,shandong,hubei,fujian,hunan,hebei,liaoning,chongqing,anhui,shanxi,
        hongkong,guangxi,yunnan,heilongjiang,shaanxi,taiwan,jilin,hainan,guizhou,tianjin,gansu,neimongol,xinjiang,qinghai,macau,xizang,time) values('{$filmname}','{$user_reyidu["ningxia"]["num"]}','{$user_reyidu["jiangxi"]["num"]}','{$user_reyidu["beijing"]["num"]}',
