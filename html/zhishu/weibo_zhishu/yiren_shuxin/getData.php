    ...
    foreach ($yiren as $key => $value) {
        ...
        $yirenname = $value[0];
        exec("python /var/www/html//zhishu/weibo_zhishu/yiren_shuxin/getId.py $yirenname", $id);
        sleep(1);
        if ($id[0] == 'error') {
            $id = NULL;
            continue;
        }
        exec("python /var/www/html//zhishu/weibo_zhishu/yiren_shuxin/getData.py $id[0] $id[1]", $json);
        sleep(1);
	...
	if(count($sex) == 1 && count($age) == 1 && count($tag) == 1 && count($star) == 2){
        ...
        //插入库
        mysqli_query($con, "insert into wzs_yiren_shuxin values('{$yirenname}','{$sex["man"]}','{$sex["woman"]}','{$age["0-12"]}','{$age["12-18"]}','{$age["19-24"]}'
