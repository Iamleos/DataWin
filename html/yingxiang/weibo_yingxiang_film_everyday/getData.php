...
//获取tv名字
    ...
    $date = date("Y-m-d");
    $data = mysqli_query($con,"select mainname from filmname where zzsy=1;");
...
//连接yingxiang数据库
    ...
    foreach ($data as $key => $value) {
        ...
        foreach ($tag as $key1 => $value1) {
            ...
            mysqli_query($con, "insert into yingxiang_film_everyday values('{$value[0]}','{$tag[$key1]}','{$size[$key1]}','{$date}');");
        }
        var_dump($value[0]);
    }
    mysqli_close($con);
