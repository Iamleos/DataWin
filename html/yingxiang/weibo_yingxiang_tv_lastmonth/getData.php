...
//连接yingxiang数据库
    ...
    $con=mysqli_connect($host,$name,$password,$dbname,4892) or die("Can't connect mysql!".mysqli_connect_error() );
    mysqli_query($con,"set names utf8");
    mysqli_query($con ,"delete from yingxiang_tv_lastmonth;");
    ...
    foreach ($tvname as $key => $value) {
        ...
        foreach ($tag as $key1 => $value1) {
            ...
            mysqli_query($con, "insert into yingxiang_tv_lastmonth values('{$value[0]}','{$tag[$key1]}','{$size[$key1]}','{$date}');");
            mysqli_query($con, "insert into yingxiang_tv_history values('{$value[0]}','{$tag[$key1]}','{$size[$key1]}','{$date}');");
        }
        var_dump($value[0]);
    }
    mysqli_close($con);
