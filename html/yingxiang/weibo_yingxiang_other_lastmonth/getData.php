...
//获取yiren名字
    ...
    $date = date("Y-m-d");
    $filmname = mysqli_query($con,"select name from other_name;");
...
//连接yingxiang数据库
    ...
    foreach ($filmname as $key => $value) {
        ...
        foreach ($tag as $key1 => $value1) {
            ...
            mysqli_query($con, "insert into yingxiang_other_lastmonth values('{$value[0]}','{$tag[$key1]}','{$size[$key1]}','{$date}');");
            mysqli_query($con, "insert into yingxiang_other_history values('{$value[0]}','{$tag[$key1]}','{$size[$key1]}','{$date}');");
        }
        var_dump($value[0]);
    }
    mysqli_close($con);
