...
mysqli_query($con, "drop table if exists dianyingweixin");
mysqli_query($con, "create table dianyingweixin(dname varchar(30),dpaper varchar(30),dread varchar(30),dlove int(8),ddate varchar(30));");
$res = mysqli_query($con, "select mname from newfilm");
$yname = array();
$i=0;
...
while(count($yname) != 0){
  ...
  foreach ($yname as $key => $value) {
    if($data[$value]['paper']!=NULL){
      mysqli_query($con, "insert into dianyingweixin(dname,dpaper,dread,dlove,ddate) values('{$data[$value]['name']}','{$data[$value]['paper']}','{$data[$value]['read']}','{$data[$value]['love']}','{$data[$value]['time']}')");
    }
    else {
