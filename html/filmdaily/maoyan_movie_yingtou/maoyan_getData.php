<?php
  ...
  $flag = 0;
  $filename = array();
  $filmnamefile = fopen("/var/www/html/filmdaily/maoyan_movie_yingtou/filmname.txt","r");
  ...
  foreach ($filmname as $key => $value) {
    $ch = curl_init("http://piaofang.maoyan.com/movie/".$value[1]."/shadowBox");
    ...
    if($ttf[1]!=0){
        ...
        $com = "wget -P /var/www/html/filmdaily/maoyan_movie_yingtou ".$ttf[1][0];
        shell_exec($com);
        sleep(3);
        shell_exec('rename '.$keyname[1][0].' map.ttf /var/www/html/filmdaily/maoyan_movie_yingtou/'.$keyname[1][0]);
        sleep(3);
        shell_exec("/home/jdk/bin/java -classpath /var/www/html/filmdaily/maoyan_movie_yingtou maoyan");
        sleep(3);
        $map = array();
        $mapfile = fopen('/var/www/html/filmdaily/maoyan_movie_yingtou/key.txt','r');
        for($i=0; $i<10; $i++){
          $map[$i] = "&#x".str_replace("\r\n","",fgets($mapfile)).";";
        }
