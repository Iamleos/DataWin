<?php
  ...
  preg_match_all('/<span id=\"dayStr\" class=\"today\">\s*(.*)\s*周/', $result, $piaofangshijian);
  ...
  mysqli_query($con, "create table mpiaofangjianbao(mname varchar(30), msumboxoffice varchar(30), mboxoffice varchar(30), mboxofficerate varchar(30), mrowpiecerate varchar(30), mseatrate varchar(30), mpiaofangshijian varchar(30), macquitime varchar(30));");
  var_dump($piaofangshijian[0]);
  for($i = 0; $i < count($filmname); $i++){
    mysqli_query($con, "insert into mpiaofangjianbao(mname, msumboxoffice, mboxoffice, mboxofficerate, mrowpiecerate, mseatrate, mpiaofangshijian, macquitime) values('{$filmname[$i]}', '{$sumboxoffice[$i]}', '{$boxoffice[$i]}', '{$boxofficerate[$i]}', '{$rowpiecerate[$i]}', '{$seatrate[$i]}', '{$piaofangshijian[0]}', '{$acquittime}')");
  }
  mysqli_close($con);
