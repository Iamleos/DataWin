<?php
    ...
    for($i = 0; $i < 45; $i++){
        ...
        $filmname = $filmname[1];
        $filmnum = $filmnum[1];
        $file = fopen("/var/www/html/filmdaily/maoyan_movie_yingtou_supplement/filmname.txt","w");
        foreach ($filmnum as $key => $value) {
        fwrite($file, $filmname[$key]." ".$value."\n");
        }
        shell_exec("php /var/www/html/filmdaily/maoyan_movie_yingtou_supplement/maoyan_getData.php $date");
    }
