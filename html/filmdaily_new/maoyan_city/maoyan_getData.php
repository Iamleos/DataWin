<?php
  ...
  $flag = 0;
  $filename = array();
  $filmnamefile = fopen("/var/www/html/filmdaily_new/maoyan_city/filmname.txt","r");
  ...
  foreach ($filmname as $key => $value) {
      $url = "http://piaofang.maoyan.com/movie/".$value[1]."/cityBox";
    $ch = curl_init($url);
    ...
    if($ttf[1]!=0){
        ...
        getWoff($url,__DIR__);
        sleep(3);
        shell_exec("/home/jdk/bin/java -classpath /var/www/html/filmdaily_new/maoyan_city maoyan");
        sleep(3);
        $map = array();
        $mapfile = fopen('/var/www/html/filmdaily_new/maoyan_city/key.txt','r');
        for($i=0; $i<10; $i++){
          $map[$i] = "&#x".str_replace("\r\n","",fgets($mapfile)).";";
        }
        ...
        foreach ($city as $key2 =>$value2) {
            mysqli_query($con,"insert into maoyan_city(name,city,piaofang,pf_rate,pp_rate,boxofficesum,seat_rate,hj_rate,people_per,people_total,session,time) values('{$value[0]}','{$value2}','{$data[$key2*9]}','{$data[$key2*9+1]}','{$data[$key2*9+2]}','{$data[$key2*9+4]}','{$data[$key2*9+5]}','{$data[$key2*9+6]}','{$data[$key2*9+3]}','{$data[$key2*9+7]}','{$data[$key2*9+8]}','{$date}');");
        }
    }
  }
  mysqli_close($con);
