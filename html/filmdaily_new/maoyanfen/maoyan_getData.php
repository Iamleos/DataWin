<?php
  ...
  $flag = 0;
  $filename = array();
  $filmnamefile = fopen("/var/www/html/filmdaily_new/maoyanfen/filmname.txt","r");
  ...
  foreach ($filmname as $key => $value) {
    ...
    getWoff($url,__DIR__);
    sleep(3);
    shell_exec("/home/jdk/bin/java -classpath /var/www/html/filmdaily_new/maoyanfen maoyan");
    sleep(3);
    $map = array();
    $mapfile = fopen('/var/www/html/filmdaily_new/maoyanfen/key.txt','r');
    for($i=0; $i<10; $i++){
      $map[$i] = "&#x".str_replace("\r\n","",fgets($mapfile)).";";
    }
