<?php
   ...
   //计算百度搜索指数的行数
   $line = count(file('/var/www/html/uploads/youyiba.txt'));

   //打开百度搜索指数文件
   $youyiba=file("/var/www/html/uploads/youyiba.txt");
   ...
   //读取电影吧中的数据
	for($j=0;$j<$line;$j++)
	{
      ...
      if($j>0)
      { //先分割一行的字符串
        $arr=explode("\t",$youyiba[$j]);
	    //获得字符串中艺人名称
        $ytitle=trim($arr[0]);
		$ysendtime=trim($arr[2]);
		$yacquitime=trim($arr[4]);

		$day=date("Y-m-d");
		$second=":00";
           if(strstr($ysendtime,":"))
           {
               $sqlinsert="insert into youyiba(ytitle,ysendtime,yacquitime) values('{$ytitle}','{$day} {$ysendtime}{$second}','{$yacquitime}')";
               echo $sqlinsert;
