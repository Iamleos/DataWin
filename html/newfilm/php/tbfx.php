 ...
 */
	...
	while($row=mysql_fetch_row($film,true)){
		$name=$row['mainname'];
		//获取指定日期的前一天
		$yesterday=date("Y-m-d",(strtotime("{$row_time[0]}")-3600*24*7));
		...
		if($film_row=mysql_fetch_row($film_yesterday,true))
		{
			...
			if($count_today!=4)
			{
				$hgd_today=round(($dfen_today*0.4+$gfen_today*0.2+$mfen_today*0.2+$wbfen_today*0.2)/(1-round($count_today/4.0,2)),2);
			}
			...
			{
				$hgd_yesterday=round(($dfen_yesterday*0.4+$gfen_yesterday*0.2+$mfen_yesterday*0.2+$wbfen_yesterday*0.2)/(1-round($count_yesterday/4.0,2)),2);
			}
