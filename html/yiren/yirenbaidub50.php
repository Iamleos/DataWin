...
#! /usr/bin/php -q
	...
	//mysql_query("drop table if exists yirenbaidub50;",$con);
    mysql_query("create table if not exists yirenbaidub50(ynum int(2),yname varchar(30),ysearch int(8),yacquitime date,primary key(yname,yacquitime));",$con);

	$url="http://top.baidu.com/buzz?b=17&fr=topbuzz_b18";
	...
	for($i=0;$i<50;$i++)
	{
		...
		$sqlstr="insert into yirenbaidub50(ynum,yname,ysearch,yacquitime) values('{$num}','{$name}','{$search}','{$acquitime}');";
